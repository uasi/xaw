...
#     http://github.com/jnthn/blizkost/blob/master/Configure.pl
#

=head1 NAME

Configure.pl - a configure script for libxml-parrot
...
open(my $fh, '< src/pmc/Makefile') or
    die "Could not open src/pmc/Makefile: $!";
my $mf = join('', <$fh>);
close $fh;
open($fh, '> src/pmc/Makefile') or
    die "Could not open src/pmc/Makefile: $!";
print $fh <<MAKEFILE;
...
You can now use `make' to build libxml-parrot.
